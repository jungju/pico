...
- 현재 틀린그림 찾기는 OpenAI Image API로 생성/편집한 자연스러운 좌우
  비교 JPG와 정확한 hitbox를 사용한 `contents` 등록 20개 stage를 제공합니다.
...
- 완료 안내는 축하 medal, 점수 카드, 다음 stage가 있으면 `Next Stage`와 `Restart`, 마지막 stage에서는 `Play Again`, 첫 화면으로 돌아가는 `Game List` 버튼을 보여줍니다.
...
- 현재 Spot content image는 `contents/<stage-id>.jpg` 한 장 안에 좌/우
  비교 그림을 함께 담는 API 생성/편집 raster pair입니다.
...
- 미로 진행률은 전체 방문 칸 수가 아니라 목표까지 가까워지는 정도와 수집한
  gem 수를 중심으로 표시합니다.
- 미로 이동 버튼은 board 아래 또는 옆에 안정적인 3x3 D-pad로 묶어 보여줍니다.
- 데스크톱 미로 화면에서는 board와 D-pad가 가까운 고정 폭 columns 안에 함께
  배치됩니다.
- 미로 player는 작은 얼굴이 있는 캐릭터 marker로 표시해 이동 주체를 분명히
  보여줍니다.
...
- 모바일에서도 stage pair 수가 정한 column 수를 유지해 4쌍 stage는 안정적인
  4x2 카드판으로 표시됩니다.
- 메모리 카드 grid는 pair 수별 최대 카드 크기를 사용하고, 모바일에서는
  padding/gap을 줄여 play area 안에서 카드를 더 크게 보여줍니다.
- 메모리 카드 뒷면은 Pico 배지와 sticker pattern이 있는 게임 카드 디자인을
  사용합니다.
- 메모리 매칭 성공 카드는 짧은 pop/sparkle과 `Match +100` 배지로 피드백을
  제공합니다.
- 메모리 힌트는 긴 단어 목록 대신 남은 pair 수를 보여주고 실제 카드 한 장을
  잠깐 강조합니다.
- 메모리 `tries`는 저장 상태와 별개의 게임 정보 chip으로 표시됩니다.
- 메모리 완료 화면은 맞춘 pair를 작은 sticker summary로 보여줍니다.
