...
- `docs/v1-feedback-policy.md`: v1 visual and speech feedback intensity policy
...
- `tools/pico-content`: Go CLI for prompt-to-Spot-stage content generation,
  image edit orchestration, pixel-diff components, bbox export, and legacy
  stage analysis/rebuild support
...
- Uses OpenAI Image API generated and edited raster pairs for the current Spot
  the Difference content set, with natural scene differences and matching JSON
  hitboxes in `contents/`.
...
- On desktop Spot the Difference screens, the shell and picture grid use a
  ...
  dominant at 1280px.
- Spot the Difference marks clickable pictures with a target affordance,
  crosshair cursor, and stronger focus glow, while reference pictures use a
  quieter eye marker and reduced emphasis.
