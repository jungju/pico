...
- [x] `V1-T014` `P0` 공통 game shell을 추출하거나 공식화한다.
  HUD, `Game List`, `Hint`, `Reset`, 계정 버튼, 학습 패널, 완료 dialog,
...
- [x] `V1-T080` `P0` 모든 게임 action에 visible label이 있는지 확인한다.
  ...
  유지한다.
  완료: 공통 game shell의 `Game List`, `Hint`, `Reset`, `Log in`, `Log out`과
  completion dialog의 `Next Stage`, `Game List`, `Keep Playing` visible label을 확인하고,
...
- [x] `V1-T103` `P0` 모든 주요 touch target을 44px 이상으로 맞춘다.
  `Log in`, `Game List`, `Hint`, `Reset`, `Speak`가 38-40px 높이로 측정된다.
...
- [x] `V1-T106` `P0` navigation/action button label을 더 명확하게 바꾼다.
  `Games`, `Home`, `Stay`는 어린아이와 보호자 모두에게 애매할 수 있다.
  `Game List`, `Keep Playing`, `Start Over`처럼 행동 결과가 분명한 label을
  검토한다.
  완료: topbar navigation은 `Game List`, completion dialog는 `Next Stage`,
  `Game List`, `Keep Playing`으로 바꿔 행동 결과를 더 분명하게 했다. Reset
  위험도와 표현은 `V1-T107`에서 별도로 다룬다.

- [x] `V1-T107` `P0` `Reset`의 실수 클릭 위험을 낮춘다.
  현재 `Hint`와 같은 무게로 붙어 있어 실수로 진행을 날릴 수 있다.
  Reset은 보조 style, 길게 누르기, confirm, 또는 overflow action으로
  분리한다.
  완료: Reset을 즉시 실행하지 않고 2.6초 안에 한 번 더 누르면 실행되는
  confirm 단계로 바꿨다. 첫 클릭 후 버튼 label과 색이 `Confirm` 상태로
  바뀌며, 시간이 지나면 자동으로 일반 Reset 상태로 돌아간다.

- [x] `V1-T108` `P1` 게임 화면의 `Log in` CTA를 더 작게 정리한다.
  로그아웃 상태에서 `Log in`이 full-width action으로 play area를 줄인다.
  저장 유도는 필요하지만 게임 중 핵심 action보다 약하게 보이게 한다.
  완료: game HUD account action의 border/background/shadow/font weight를 낮춰
  `Log in`/`Log out`이 핵심 play action보다 조용한 보조 CTA로 보이게 했다.
  버튼 크기는 44px touch target을 유지한다.

- [x] `V1-T109` `P1` progress meter를 더 읽기 쉽게 만든다.
  0% 상태에서는 얇은 빈 선처럼 보여 의미가 약하다. 두께, 시작 marker,
  label, 완료 animation을 조정하고 게임 타입별 progress 의미를 맞춘다.
  완료: progress meter를 더 두껍게 만들고 start marker, subtle tick track,
  fill end marker를 추가했다. meter에는 `progressbar` semantics와 현재
  percent 값을 제공한다.

- [x] `V1-T110` `P1` 완료 dialog를 더 축하스럽고 게임답게 만든다.
  현재 `Success`, 점수, 기본 action만 보여 다소 건조하다. 별, confetti,
  score breakdown, streak reward, 다음 행동 hierarchy를 추가한다.
  완료: completion dialog에 trophy medal, static confetti, `Great job!`
  headline, 점수 카드 visual을 추가했다. 버튼 hierarchy의 세부 정리는
  `V1-T111`에서 계속 다룬다.

- [x] `V1-T111` `P1` 완료 dialog button hierarchy를 재검토한다.
  다음 stage가 없을 때 `Game List`와 `Keep Playing`만 있어 흐름이 약하다.
  `Game List`, `Play Again`, `Keep Playing` 중 어떤 행동이 주 행동인지 정한다.
  완료: 다음 stage가 있으면 `Next Stage`를 primary로 유지하고, 다음 stage가
  없으면 `Play Again`을 primary로 제공한다. `Game List`와 `Keep Playing`은
  보조 action으로 유지한다.

- [x] `V1-T112` `P1` motion과 animation의 reduce-motion 대응을 추가한다.
  hint pulse, marker pulse, completion feedback은 `prefers-reduced-motion`에서
  줄이거나 정지한다.
  완료: `prefers-reduced-motion: reduce`에서 hint button/marker pulse,
  maze nudge, learning panel pop, auth spinner animation을 정지하고 주요
  transition을 제거했다. Hint 강조는 static shadow로 유지한다.

- [x] `V1-T113` `P1` 색만으로 상태를 구분하지 않게 보강한다.
  found, hinted, wrong, complete 상태가 색에 많이 의존한다. icon, pattern,
  shape, label 변화를 함께 사용한다.
  완료: correct/found marker에 check shape를 추가하고, Hidden Objects target
  pill에 `Found`/`Hint` badge를 표시했다. Maze visited cell은 dot marker를,
  Memory matched card는 `Match` badge를 함께 보여준다.

- [x] `V1-T120` `P0` 320px 홈 카드의 한국어 줄바꿈을 고친다.
  `아이 방`, `소풍 놀이터`, `동물 카드`가 글자 단위로 끊겨 보인다.
  한국어 title을 별도 줄이나 pill로 분리하고 `word-break`/width를 조정한다.
  완료: 홈 stage card에서 game type과 한국어 stage title을 분리하고,
  한국어 title은 `lang="ko"`와 `word-break: keep-all`이 적용된 별도 pill로
  표시한다. 320px screenshot에서 한국어가 글자 단위로 끊기지 않음을
  확인했다.

- [x] `V1-T121` `P0` 홈 카드 정보 hierarchy를 정리한다.
  제목, game type, 한국어 title, level, theme가 한 카드 안에서 경쟁한다.
  title, game type, metadata의 시각 순서를 고정하고 사소한 metadata는 더
  작은 badge로 낮춘다.
  완료: stage card hierarchy를 작은 game type eyebrow, 큰 stage title,
  한국어 title pill, 작은 level/theme metadata badge 순서로 정리했다.
  metadata badge의 색과 크기를 낮춰 제목 경쟁을 줄였다.

- [x] `V1-T122` `P0` 80개 stage를 위한 game type filter를 추가한다.
  v1 목표가 총 80개이므로 단순 세로 list는 탐색 비용이 커진다. Spot,
  Hidden, Maze, Memory tab 또는 segmented control을 제공한다.
  완료: 홈 화면에 All/Spot/Hidden/Maze/Memory segmented filter를 추가하고
  각 filter에 stage count를 표시한다. 선택한 game type만 stage list에
  남도록 좁힌다.

- [x] `V1-T123` `P0` stage가 많아질 때 level/theme 탐색을 지원한다.
  Level, theme, 완료 여부로 정렬하거나 필터링할 수 있어야 한다.
  완료: 홈 화면에 Level, Theme, Status select filter를 추가했다. Status는
  `pico-progress.stages[stageId].completed`를 기준으로 `Open`/`Done`을
  필터링하며, 조건에 맞는 stage가 없으면 empty state를 보여준다.

- [x] `V1-T124` `P0` 홈 카드에 완료/미완료 상태를 표시한다.
  로그인 사용자는 어떤 stage를 끝냈는지, 이어서 할 stage가 무엇인지
  카드에서 바로 알 수 있어야 한다.
  완료: 로그인 사용자에게 stage card metadata 줄에 `Done`/`Open` badge를
  표시한다. badge는 `pico-progress.stages[stageId].completed`를 기준으로
  계산하고 Status filter와 같은 데이터 소스를 사용한다.

- [x] `V1-T125` `P1` `Today` CTA의 의미를 더 명확히 한다.
  현재 `TODAY Animal Match`만 보여 왜 추천되는지, 시작 버튼인지 약하다.
  `Play Today`, 추천 이유, 완료 여부를 짧게 표현한다.
  완료: CTA label을 `Play Today`로 바꾸고 `Start` action pill, `Open`/`Done`
  상태, 추천 이유를 함께 표시한다. 로그인 사용자에게는 저장된 완료 상태를
  기준으로 다음 `Open` stage를 우선 추천한다.

- [x] `V1-T126` `P1` 홈 카드 preview asset의 일관성을 높인다.
  Memory preview는 실제 카드 게임 느낌이 약하고, 일부 stage preview는
  밀도가 다르다. game type별 thumbnail framing과 visual style 기준을
  만든다.
  완료: 홈 stage card thumbnail에 game type class를 추가하고, Spot/Hidden은
  edge-to-edge scene, Maze/Memory는 inset board/card tray로 보이게 프레이밍을
  분리했다. `docs/v1-assets.md`에 86px/124px 기준 thumbnail 규칙을 고정했다.

- [x] `V1-T127` `P1` 320px 홈 카드 높이를 줄인다.
  좁은 화면에서 첫 카드가 약 190px까지 커지고 badge가 세로로 쌓인다.
  thumbnail, title, badge 배치를 더 compact하게 조정한다.
  완료: 480px 이하에서 stage card thumbnail을 64px로 줄이고, arrow column,
  gap, title, Korean pill, metadata badge를 compact하게 조정했다. 320px
  검수에서 긴 Spot 카드 높이가 182px에서 약 106px 수준으로 줄었다.

- [x] `V1-T128` `P1` 데스크톱 홈 화면의 stage list 밀도를 개선한다.
  1280px에서도 한 줄 list만 사용해 화면 아래로 길어진다. 80개 stage 전에는
  2-column grid, grouped section, 또는 paged list를 검토한다.
  완료: 홈 selector 최대 폭을 1120px로 넓히고, 980px 이상에서 stage list를
  2-column grid로 전환했다. 데스크톱 카드 thumbnail/title 크기는 2-column
  폭에 맞춰 재조정하고 empty state는 전체 grid 폭을 차지하게 했다.

- [x] `V1-T129` `P1` 홈에서 game type별 visual identity를 강화한다.
  카드의 색, icon, badge가 game type을 즉시 말해주도록 Spot/Hidden/Maze/
  Memory별 accent를 적용한다.
  완료: 홈 stage card에 game type class와 accent stripe를 추가하고, type label을
  game type별 icon badge로 바꿨다. Spot/Hidden/Maze/Memory는 각각 다른 accent
  color, soft badge background, icon을 사용한다.

- [x] `V1-T130` `P1` 로그아웃 홈에서 저장 가치 설명을 작고 명확하게 만든다.
  로그인하지 않아도 플레이 가능하지만, point/streak 저장은 로그인 후
  가능하다는 차이를 부담 없이 알린다.
  완료: anonymous/error 상태의 홈 header 아래에 작은 save-progress note를
  추가했다. `Play now. Log in to save points and streaks.` copy와
  `Save progress` login action을 제공하고, authenticated 상태에서는 숨긴다.

- [x] `V1-T140` `P0` Spot 모바일 play area의 빈 세로 여백을 줄인다.
  이미지 위아래 여백이 커서 실제 비교 그림이 작게 보인다. 두 그림은
  좌우 유지하되 available height를 더 적극적으로 사용한다.
  완료: `GameShell`에 game type class를 붙이고, Spot 모바일 shell은 picture
  area를 content-height row로 배치한다. 좌우 비교는 유지하면서 pictures grid의
  padding/gap을 줄이고 frame width를 cell에 맞췄다. 390px 기준 frame은
  175x263에서 182x273으로 커졌고, grid 내부 위아래 blank는 약 166px에서
  8px로 줄었다.

- [x] `V1-T141` `P0` Spot 데스크톱 그림 크기와 gutter를 재조정한다.
  1280px 화면에서 stage panel 양쪽 빈 공간이 크다. 이미지 최대 폭을 키우거나
  panel width를 줄여 비교 대상이 더 크게 보이게 한다.
  완료: desktop Spot shell을 1080px focused width로 줄이고, pictures grid
  column 상한을 470px로 늘리며 padding/gap을 조정했다. 1280px 검수에서 frame은
  430x645에서 453x680으로 커지고 좌우 gutter는 183px에서 약 72px로 줄었다.

- [x] `V1-T142` `P0` Spot의 클릭 가능한 그림과 참고 그림 affordance를
  구분한다.
  `original/changed` label은 쓰지 않더라도, 클릭 가능한 쪽은 subtle focus
  frame이나 cursor feedback으로 더 명확해야 한다.
  완료: clickable picture button에는 target marker, crosshair cursor, stronger
  focus/hover glow를 적용하고 reference picture에는 eye marker와 낮은 강조도를
  적용했다. screen reader label도 clickable/reference 역할이 드러나게 바꿨다.

- [x] `V1-T143` `P1` Spot에 확대/집중 보기 affordance를 추가한다.
  모바일 좌우 비교는 차이가 작아질 수 있다. 긴 누름 zoom, tap-to-inspect,
  또는 일시 확대 모드를 검토한다.
  완료: picture area에 44px `Focus` affordance를 추가하고 focus view dialog를
  구현했다. 모바일은 큰 그림을 세로로 쌓아 inspect하고, 데스크톱은 좌우 비교
  overlay를 유지한다. Escape, overlay click, close button으로 닫을 수 있다.

- [x] `V1-T144` `P1` Spot correct/hint marker 크기를 target별로 조정한다.
  marker가 작은 차이를 덮거나 큰 차이에 비해 작아 보일 수 있다. bbox/area
  크기를 반영한 marker scale을 검토한다.
  완료: correct/hint marker가 difference area bounds를 읽어 width/height를
  `clamp(30px, area%, 64px)`로 계산한다. circle/rect/polygon area를 모두
  지원하며 marker stroke와 check shape도 marker 크기에 맞춰 상대 배치한다.

- [x] `V1-T145` `P1` Spot 기본 learning panel 내용을 더 목적 있게 만든다.
  현재 단어 list가 길게 나열된다. 첫 진입 시 stage 목표, 최근 찾은 단어,
  또는 짧은 vocabulary chips 중심으로 정리한다.
  완료: Spot ready message를 stage title 반복 대신 `Goal: n differences`로
  바꾸고, body는 `Words:` prefix가 있는 핵심 vocabulary list로 정리했다.

- [x] `V1-T146` `P1` Spot wrong feedback을 더 부드럽고 유용하게 만든다.
  단순 `Wrong`보다 아이에게 부담이 적은 copy와 시각 feedback을 사용하고,
  반복 오답 시 hint로 자연스럽게 이어지게 한다.
  완료: wrong message를 `Try again`으로 바꾸고 첫 오답은 부드러운 재시도,
  반복 오답은 hint 준비 상태를 알려주는 copy로 분기했다. 정답/hint/reset 시
  wrong streak를 초기화하고 wrong panel color를 더 부드러운 orange 계열로 낮췄다.

- [x] `V1-T150` `P0` Hidden target pill의 text clipping을 해결한다.
  320/390px에서 target pill 내부 text line-height가 scroll height보다 작게
  측정된다. pill 높이, line-height, font size를 안정화한다.
  완료: target word/meaning을 block line으로 고정하고 line-height/min-height를
  명시했다. 모바일 pill은 44px 높이, 6px vertical padding으로 올리고 list
  max-height를 146px로 조정해 320/390px에서 text clientHeight와 scrollHeight가
  일치하도록 했다.

- [x] `V1-T151` `P0` Hidden target list의 8개 target 확장 상태를 설계한다.
  v1 조건은 3-8개 target인데 모바일 현재 list는 6개 기준이다. 8개에서도
  scroll affordance, 완료 표시, hint 강조가 선명해야 한다.
  완료: target count가 7개 이상이면 `hidden-target-list many-targets` 상태와
  `data-target-count`를 붙인다. 모바일 many-targets는 196px/24dvh 기준
  max-height, stable scrollbar gutter, thin scrollbar, bottom fade를 사용해
  8개 target에서 스크롤 가능함을 보여준다. 기존 `Found`/`Hint` 상태 badge는
  그대로 유지된다.

- [x] `V1-T152` `P1` Hidden scene image crop 정책을 검토한다.
  `object-fit: cover`는 장면 가장자리 target을 잘라낼 수 있다. target이
  edge에 있을 때 contain/framing/padding 기준을 정한다.
  완료: Hidden scene frame은 stage data의 scene aspect ratio를 유지하고,
  scene image는 `object-fit: contain`으로 렌더링한다. 이 조합을 crop 방지
  정책으로 문서화했다.

- [x] `V1-T153` `P1` Hidden marker visibility를 높인다.
  밝고 복잡한 scene에서 found/hint marker가 묻힐 수 있다. marker contrast,
  halo, pulse, label 없는 상태 표시를 개선한다.
  완료: Hidden scene marker에 반응형 크기와 흰 halo, game color halo를
  추가했다. hint marker는 solid border와 밝은 cyan fill을 써서 밝은 scene에서도
  눈에 띄며, found marker는 check shape를 유지한 채 green halo contrast를
  강화했다.

- [x] `V1-T154` `P1` Hidden target pill의 found/hint 상태를 더 명확히 한다.
  색 변화만으로는 찾은 것과 힌트 대상이 약할 수 있다. check icon, target
  icon, outline pattern을 함께 쓴다.
  완료: Hidden target pill에 lucide check/target icon과 `Found`/`Hint` 상태
  label을 명시적으로 렌더링했다. 찾은 target은 stripe pattern과 solid outline,
  hint target은 dotted pattern과 dashed outline을 사용해 색상 없이도 상태가
  구분된다.

- [x] `V1-T155` `P1` Hidden 오답 feedback 위치를 scene 근처로 보강한다.
  현재 message panel만 바뀌면 클릭 지점과 feedback의 연결이 약하다. scene
  안 wrong marker나 작은 shake를 추가한다.
  완료: Hidden scene 오답 클릭 지점에 짧게 사라지는 X marker를 표시하고,
  scene에는 작은 nudge animation을 추가했다. 정답/리셋 시 wrong marker는
  즉시 사라지며, reduce-motion 환경에서는 반복 움직임을 정지한다.

- [x] `V1-T160` `P0` Maze progress 표시를 goal 중심으로 바꾼다.
  `1/32`는 모든 칸을 방문해야 하는 것처럼 보인다. `Goal`, `2 gems`, 또는
  path progress처럼 미로 규칙과 맞는 지표를 사용한다.
  완료: Maze HUD의 progress text를 방문 칸 수 대신 `Goal · n/m gems`로 바꿨고,
  meter는 시작점-목표 최단거리와 현재 위치-목표 최단거리 차이를 기준으로
  채워지게 했다. 목표에 도착하면 100%가 된다.

- [x] `V1-T161` `P0` Maze 모바일 D-pad 배치를 고친다.
  up button이 board 경계에 붙어 떠 있고, control 묶음이 분리돼 보인다.
  네 방향 버튼을 하나의 안정적인 D-pad로 묶어 board 아래에 배치한다.
  완료: Maze movement controls를 3x3 D-pad 구조로 재배치하고 중앙 hub,
  spacer, 48px direction button을 추가했다. 모바일에서는 board 아래에 하나의
  control pad로 붙어 보이도록 padding, background, shadow를 정리했다.

- [x] `V1-T162` `P0` Maze 데스크톱 board와 control의 거리감을 줄인다.
  데스크톱에서 board는 왼쪽, controls는 오른쪽 빈 공간에 떨어져 있다.
  controls를 board 가까이에 붙이거나 board 하단으로 통일한다.
  완료: 980px 이상 Maze stage는 board 620px column과 D-pad 176px column을
  가운데 정렬하고, board는 오른쪽 끝, D-pad는 왼쪽 끝으로 붙여 두 요소 사이
  거리를 stage gap 수준으로 유지한다.

- [x] `V1-T163` `P0` Maze player를 아이 친화적인 캐릭터로 바꾼다.
  현재 red dot은 출발 캐릭터인지 선택 marker인지 약하다. 작은 캐릭터,
  얼굴, 발자국 등으로 이동 주체를 분명히 한다.
  완료: Maze player marker를 단색 red dot에서 얼굴이 있는 둥근 캐릭터로
  바꿨다. 눈/미소 highlight와 더 큰 marker 크기를 적용하고, screen reader용
  `Player` label을 추가했다.

- [x] `V1-T164` `P1` Maze start/goal/collectible affordance를 키운다.
  flag와 gem icon이 작아 목표와 보너스가 눈에 잘 띄지 않는다. icon 크기,
  contrast, cell background를 조정한다.
  완료: start/goal/collectible cell에 전용 class와 강조 background를 추가하고,
  icon 크기를 cell 기준 `clamp(22px, 48%, 34px)`로 키웠다. gem 색상도 더 짙게
  조정해 작은 화면에서도 목표와 보너스가 더 잘 보인다.

- [x] `V1-T165` `P1` Maze wall/path contrast와 visited path 표현을 조정한다.
  길, 벽, 방문 칸, 목표 칸이 한눈에 규칙으로 읽히게 색과 pattern을
  정리한다.
  완료: open path는 밝은 cream tile, wall은 진한 blue diagonal pattern,
  visited path는 cyan tile과 짧은 route dash로 구분했다. visited rule이
  start/goal/collectible 강조를 덮지 않도록 selector를 분리했다.

- [x] `V1-T166` `P1` Maze touch gesture 안내를 개선한다.
  board tap, direction button, drag/swipe 중 어떤 입력이 가능한지 시각적으로
  알기 어렵다. visible control affordance와 feedback을 강화한다.
  완료: 현재 player 주변의 이동 가능한 인접 칸에 `next-step` outline과 작은
  marker를 표시하고, board/open cell cursor와 D-pad active feedback을 추가했다.
  별도 설명 문구 없이 board tap과 방향 버튼 조작 가능성이 드러난다.

- [x] `V1-T170` `P0` Memory 모바일 card grid를 균형 있게 만든다.
  390px에서 8장이 5+3 배열로 보여 카드 게임판답지 않다. 4쌍 stage는
  4x2 또는 2x4처럼 안정적인 grid를 사용한다.
  완료: 모바일 CSS의 `auto-fit` override를 제거하고 stage에서 계산한
  `--memory-columns`를 그대로 사용하게 했다. 4쌍/8장 stage는 모바일에서도
  안정적인 4x2 board로 표시된다.

- [x] `V1-T171` `P0` Memory card 크기를 play area에 맞춰 키운다.
  모바일과 데스크톱 모두 카드가 stage panel에 비해 작고 빈 공간이 많다.
  pair 수별 최대 카드 크기와 grid width를 재정의한다.
  완료: deck size별 `--memory-card-max`를 도입해 8장/12장/16장/24장 board의
  최대 카드 크기를 다르게 잡았다. grid width 계산은 해당 max size와 gap을
  사용하며, 모바일 memory stage padding/gap을 줄여 같은 column 수 안에서 카드
  크기를 조금 더 확보했다.

- [x] `V1-T172` `P1` Memory card back을 더 게임답게 디자인한다.
  현재 파란 카드와 노란 점은 장난감 카드 느낌이 약하다. Pico 문양,
  별/스티커, game type accent를 넣은 card back asset을 만든다.
  완료: Memory card back에 Pico `P` badge, star sticker, dot sticker,
  diagonal pattern, inset shadow를 CSS로 추가했다. 별도 asset 없이 game card
  느낌을 강화했다.

- [x] `V1-T173` `P1` Memory match feedback을 더 명확히 만든다.
  matched state가 색 변화 중심이다. 카드 bounce, sparkle, pair label, 짧은
  score pop을 추가한다.
  완료: matched card에 짧은 pop animation과 star sparkle pseudo element를
  추가하고, 상태 badge를 `Match +100`으로 렌더링했다. reduce-motion 환경에서는
  match animation을 정지한다.

- [x] `V1-T174` `P1` Memory hint 동작을 재설계한다.
  현재 hint는 match mode와 모든 단어를 message panel에 나열한다. 실제 게임
  도움으로 느껴지게 잠깐 카드 한 장 강조, 남은 pair count, 쉬운 pair 추천
  등을 검토한다.
  완료: Hint를 누르면 남은 pair 수를 message panel에 표시하고, 아직 맞추지 않은
  카드 한 장을 1.5초 동안 강조한다. 카드가 한 장 열려 있으면 해당 카드의
  matching card를 우선 강조해 실제 플레이 도움으로 이어지게 했다.

- [x] `V1-T175` `P1` Memory attempts 표시 위치를 정한다.
  로그인 상태에서는 `tries`가 저장 상태 chip을 대체하고, 로그아웃에서는
  보이지 않는다. tries는 게임 정보로 별도 표시한다.
  완료: 공통 `GameShell`에 선택적 `infoText` chip을 추가하고 Memory tries를
  이 chip으로 표시했다. Memory의 저장 상태는 다시 `Ready`/`Local play`로
  유지되어 시도 횟수와 저장 상태가 분리된다.

- [x] `V1-T176` `P1` Memory 완료 화면에 matched card 결과를 보여준다.
  완료 dialog 뒤에 카드가 흐릿하게 보이지만, 성공 결과 자체는 text뿐이다.
  맞춘 동물/단어 summary나 sticker reward를 보여준다.
  완료: `GameShell` completion dialog에 optional summary 영역을 추가하고,
  Memory 완료 시 맞춘 pair를 최대 6개 sticker로 보여준다. 이미지/emoji/word
  face를 지원하고 pair가 더 많으면 `+n` sticker를 표시한다.

- [x] `V1-T180` `P0` 화면 검수 체크리스트를 QA 문서에 고정한다.
  viewport, route, touch target, text clipping, overflow, completion dialog,
  game type별 핵심 interaction을 반복 검수할 수 있게 문서화한다.
  완료: `docs/v1-visual-qa.md`에 반복 사용 가능한 화면 QA 체크리스트를 추가했다.
  viewport, route, global UI, text/overflow, completion dialog, game type별
  핵심 interaction 기준을 고정했다.

- [x] `V1-T181` `P1` layout smoke check를 자동화한다.
  최소 viewport에서 horizontal overflow, clipped text, 44px 미만 touch target,
  play area 비율을 감지하는 Playwright 기반 smoke check를 검토한다.
  완료: `scripts/layout-smoke-check.mjs`와 `npm run smoke:layout`을 추가했다.
  Playwright로 홈과 네 게임 첫 route를 320, 390, tablet, desktop viewport에서
  열고 horizontal overflow, 44px 미만 touch target, 세로 text clipping,
  topbar/play area 비율을 점검한다.

- [x] `V1-T182` `P1` 80개 stage 기준 selector 성능과 스크롤 UX를 검증한다.
  카드 수가 늘었을 때 첫 화면 로딩, 스크롤 길이, keyboard navigation,
  focus order가 유지되는지 확인한다.
  완료: 홈 selector를 80개 카드 상태에서 390x844와 1280x900으로 측정해
  `docs/v1-manual-qa.md`에 기록했다. 모바일은 1-column `9825px`, 데스크톱은
  2-column `6301px` scroll height였고 horizontal overflow는 없었다. Today CTA,
  game type filter, Level/Theme/Status filter에는 명확한 focus label을 보강했다.

- [x] `V1-T183` `P1` mobile browser chrome 변동을 다시 확인한다.
  `100dvh` 기반 game shell이 iOS/Android 주소창 변화에서 panel을 자르지
  않는지 실제 기기 또는 emulation으로 검수한다.
  완료: Playwright viewport resize를 browser chrome 변화 proxy로 사용해 네
  게임 첫 route를 390x640, 390x740, 390x844에서 확인했다. 모든 route에서
  shell과 learning panel이 viewport 안에 머물렀고 horizontal overflow는 없었다.
  결과는 `docs/v1-manual-qa.md`에 기록했다.

- [x] `V1-T184` `P2` 게임 화면 내 copy 언어 정책을 정리한다.
  stage title은 한국어, subtitle/button은 영어가 섞여 있다. 아이용 게임으로
  어떤 label은 영어 학습용이고 어떤 label은 조작용인지 기준을 만든다.
  완료: `docs/v1-korean-copy.md`에 game screen language policy를 추가했다.
  stage title, subtitle, target word, meaning, control, status, feedback,
  completion action별 한국어/영어 사용 기준을 표로 정리했다.

- [x] `V1-T185` `P2` game UI sound/visual feedback 강도 기준을 정한다.
  정답, 오답, hint, 완료, streak reward가 과하거나 약하지 않게 feedback
  timing과 반복 빈도를 정의한다.
  완료: `docs/v1-feedback-policy.md`를 추가해 correct, wrong, hint,
  collectible, complete, streak/reward feedback의 시각/음성 timing, 반복 기준,
  motion/audio 제한, game type별 주의점, QA 질문을 정리했다.
